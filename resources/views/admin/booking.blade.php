  ...
  <main id="main" class="main">
    ...
    <section class="section">
      <div class="container">
        ...
        <div class="row">
          <div class="col-md-4">
            <div class="card info-card d-flex h-100">
              <div class="card-body d-flex align-items-center">
                <div class="d-flex flex-column flex-grow-1 text-center">
                  <h4 class="card-title">Today's Bookings</h4>
                  <h6 id="todaysBookings" class="mb-0">{{ $todaysBookings }}</h6>
                  <p class="mb-0" style="font-size: 1rem;">Number of bookings for today</p>
                </div>
                <div class="card-icon d-flex align-items-center justify-content-center ml-3">
                  <span id="todaysDate" class="font-weight-bold">{{ now()->format('Y-m-d') }}</span>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4">
            <div class="card info-card d-flex h-100">
              <div class="card-body d-flex align-items-center">
                <div class="d-flex flex-column flex-grow-1 text-center">
                  <h4 class="card-title">This Month's Bookings</h4>
                  <h6 id="monthBookings" class="mb-0">{{ $monthBookings }}</h6>
                  <p class="mb-0" style="font-size: 1rem;">Number of bookings for this month</p>
                </div>
                <div class="card-icon d-flex align-items-center justify-content-center ml-3">
                  <span id="monthDate" class="font-weight-bold">{{ now()->format('F Y') }}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="card info-card d-flex h-100">
              <div class="card-body d-flex align-items-center">
                <div class="d-flex flex-column flex-grow-1 text-center">
                  ...
                  <h6 id="totalBookings" class="mb-0">{{ $totalBookings }}</h6>
                  <p class="mb-0" style="font-size: 1rem;">Total number of bookings</p>
                </div>
                <div class="card-icon d-flex align-items-center justify-content-center ml-3">
                  <span id="totalDate" class="font-weight-bold"></span>
                </div>
              </div>
            </div>
          </div>
        ...
        <!-- Uncomment Booking Details Table if needed -->
        <div class="row mt-4">
          <div class="col-lg-12">
    ...
    <div class="modal fade" id="deleteModal{{ $booking->id }}" tabindex="-1"
      ...
      <div class="modal-content">
        <div class="modal-header" style="background-color: #f5f5f5; color: #000000;">
        <h5 class="modal-title" id="deleteModalLabel{{ $booking->id }}">Delete Booking Details</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
        <p>Are you sure you want to delete the booking for: <strong>{{ $booking->customer_name }}</strong>?</p>
        <input type="hidden" name="booking_id" value="{{ $booking->id }}">
        </div>
        <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <form action="{{ route('admin.deleteBooking') }}" method="POST" style="display:inline;">
          @csrf
          <input type="hidden" name="booking_id" value="{{ $booking->id }}">
